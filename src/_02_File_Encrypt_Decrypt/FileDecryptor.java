...
public class FileDecryptor {
	 ...
	 * understand.
	 *
	 * The shift cipher is decrypted by using the key and shifting back up, at
	...
	public static void main(String[] args) {
		String decrypted = "";
		String key = JOptionPane.showInputDialog(null, "type in a key");
		...
		try {
			...
			while (c != -1) {
				...
				decrypted+=fd.decrypt((char)c, actualKey);
				c = fr.read();
			}
			fr.close();
	...
	public char decrypt(char x, int k) {
		int ascii = (int)x; 
		int key = k%26;
		int decoded = ascii - key;
		
		if ((int)Character.toLowerCase(x) - key < (int)'a') {
			
			decoded = ascii - key + 26;
